...
[AddComponentMenu("NGUI/Interaction/Center Scroll View on Child2")]
public class IUICenterOnChild : MonoBehaviour
{
    ...
    void Start() { Recenter(); }
    void OnEnable() { if (mScrollView) { mScrollView.myCenterOnChild = this; Recenter(); } }
    void OnDisable() { if (mScrollView) mScrollView.myCenterOnChild = null; }
    ...
    public void Recenter()
    {
        if (mScrollView == null)
        {
            ...
            else
            {
                if (mScrollView)
                {
                    mScrollView.myCenterOnChild = this;
        ...
        Vector3 panelCenter = (corners[2] + corners[0]) * 0.5f;////(corners[2] + corners[0]) * 0.5f;
                ...
                if (Mathf.Abs(delta) > nextPageThreshold)
                {
                    ...
                    if (grid != null && grid.sorting != UIGrid.Sorting.None)
                    {
                        ...
                        else if (delta < -nextPageThreshold)
                        {
                           ...
                           // if (index < list.Count - 2) closest = list[index];
                           // else closest = list[list.Count - 2];
                        }
                    }
